...
!-----[--.----+----.----+----.-----------------------------------------]
      ...
      character  (len=80) :: f_pt, s_pt, c_pt
...
!         Set files for output of point data
          ...
          f_pt = 'F_ '
          f_pt(3:11-i) = fext(i:8)
          s_pt = 'sigma_ '
          s_pt(7:15-i) = fext(i:8)
          c_pt = 'moduli_ '
          c_pt(8:16-i) = fext(i:8)

          open(unit=90, file=f_pt, status='unknown')
          open(unit=91, file=s_pt, status='unknown')
          open(unit=92, file=c_pt, status='unknown')
          rewind 90
          rewind 91
          rewind 92
        else

!$Id:$
      subroutine flu2d6(d,ul,xl,ix,tl,s,r,ndf,ndm,nst,isw)
...
!             "Taylor-Galerkin/CBS modifications from ZT3n7" & RLT Note
...
!-----[--.----+----.----+----.-----------------------------------------]
      ...
      real (kind=8) :: rhoj, tlen, fact
      real (kind=8) :: conv(3), bf(3), uni(2), tt(2),nvec(2)
      real (kind=8) :: pg(2)
      real (kind=8) :: ru(2), rp(2)
...
!     Output element type
      if(isw.eq.0 .and. ior.lt.0) then
        ...
        write(*,*) '   FLU2d6: 2-d CBS/Taylor-Galerkin split QN/QN'
...
!         Shape functions and derivatives
          do l = 1,lint
!           Shape functions for velocity & pressure
...
!         Step 2: Compute pressure (assume u^T n = 0)
     ...
     &                         + ul(5,i,1)*shp2(2,i,l)
              ...
              do i = 1,nel
                r(3,i) = r(3,i) - shp2(1,i,l)*pg(1)
     &                          - shp2(2,i,l)*pg(2)
     &                          - shp2(3,i,l)*div_v
                do j = 1,nel
                  s(sa(i)+3,sa(j)+3) = s(sa(i)+3,sa(j)+3)
     &                               +(shp2(1,i,l)*shp2(1,j,l)
     &                               + shp2(2,i,l)*shp2(2,j,l))
     &                               * jac(l)
                end do ! j
              end do ! i
            end do ! l
...
!               Stabilizing term
                div_v = gradv(1,1,1,l) + gradv(2,2,1,l)
                do i = 1,ndf
                  ru(i) = uu(1,1,l)*gradv(i,1,1,l)
     &                  + uu(2,1,l)*gradv(i,2,1,l)
     &                  + uu(i,1,l)*div_v
                end do ! i
                ru(:) = 0.5d0*dt*rhoj*ru(:)
...
!               Stabilizing addition
                do i = 1,nel
                  fact = shp2(3,i,l)*div_v
     &                 + shp2(1,i,l)*uu(1,1,l)
     &                 + shp2(2,i,l)*uu(2,1,l)
                  r(4:5,i) = r(4:5,i) + ru(:)*fact
                end do ! i
...
!               9-node element
                if(nel.eq.9) then
                  do i = 1,nel
                    pp    = pp    + ul(3,i,1)*shp2(3,i,l)
                    rp(:) = rp(:) + ul(3,i,1)*shp2(1:2,i,l)
                    div_v = div_v + ul(1,i,1)*shp2(1,i,l)
     &                            + ul(2,i,1)*shp2(2,i,l)
                  end do ! i
                endif
                pp    = pp * jac(l)
                rhoj  = d(4)*jac(l)/dt
                rp(:) = 0.5d0*dt*rp(:)*jac(l)
                bf(:) = bf(:)*jac(l)
...
2000  format(5x,'C B S    A l g o r i t h m    P a r a m e t e r s')

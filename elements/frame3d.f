...
!-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    le,dl,dl2,dl3,dn,ctan1,ctan3

      integer   ix(*)
      real*8    d(*),xl(ndm,*),ul(ndf,nen,*),s(nst,*),r(ndf,*)
      real*8    b(6),t(3,3),fi(6,2),sm(12,12),pm(12)
...
!     Frame mass matrix
      ...
      real*8    cfac,lfac,ra,le,lr,t,dv,s1,s2,s3
      real*8    d(*), r(ndf,*),s(nst,nst),sg(4),ag(4),bb(2,2),db(2,2)
      real*8    nt(2,6,2),nr(3,6,2),ntak(2,6),nrik(3,6),in(3,3)
...
!     Consistent mass matrix
      ...
      do ll = 1,4
        ...
        dv     = t*ag(ll)
        in(1,1)  = d(4)*d(36)*0.5d0*le*ag(ll)
        in(2,2)  = d(4)*d(33)*0.5d0*le*ag(ll)
        in(2,3)  = d(4)*d(35)*0.5d0*le*ag(ll)
        in(3,2)  = d(4)*d(35)*0.5d0*le*ag(ll)
        in(3,3)  = d(4)*d(34)*0.5d0*le*ag(ll)

        s1     = 0.5d0 + 0.5d0*sg(ll)
        s2     = s1*s1
        s3     = s1*s2

...
!-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    thk, thk3, q0, xx, yy , xsj, ctan1
      ...
      real*8    eps(3), sig(6), dd(3,3), ds(2,2), alphg(3), co(4),si(4)
...
!                         shear(1)=q_x,shear(2)=q_y]
      ...
      integer   i,j,ndf
      real*8    dd(3,3)    , eps(3)   , sig(6)   , shear(2)
...
!-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    den, area, ar3, ar24, third, xx, yy, thk, thk3, psi
      real*8    ctan1,ctan3
      ...
      real*8    alphm(3), epsc(3),eps(3,3),sigc(6),sig(3,3)
...
!         Output moments and curvatures
          ...
          write(iow,2002) n,ma,(sigc(j),j=1,5),xx,yy,epsc,sigc(6),
     &                    shear
          if(ior.lt.0) then
            write(*,2002) n,ma,(sigc(j),j=1,5),xx,yy,epsc,sigc(6),
     &                    shear
          endif
...
!     Lumped projection routine
      ...
      integer   ix(*)
      real*8    xg, errav
      real*8    dt(numnp),st(numnp,*),ser(*),sig(3,*),eps(3,*),dv(*)
      real*8    shear(2), gamm(2), el(3,*)
...
!     Convert to global thermal stiffness vector
      ...
      real*8    area, ar3,ar24, ccm,clm, den, third

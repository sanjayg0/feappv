...
!-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    lm,cmd,cmo,hmd,hmo,geo,forc, alphar, one3

      real*8    db(3,2),bl(3,2),br(3,2),xx(3),th(2)
...
!       Output element force/strains
        ...
        if(isw.eq.4) then
          ...
          write(iow,2002) n,ma,xx,sig(1),eps(1)
          if(ior.lt.0) then
            write(*,2002) n,ma,xx,sig(1),eps(1)
          endif
        else
...
!     Compute constitutive equation
      ...
      real*8    xlen,xlen0,xlenn,xlen1,xlena,dx,du,dd, ta
      real*8    alpha, alphar
